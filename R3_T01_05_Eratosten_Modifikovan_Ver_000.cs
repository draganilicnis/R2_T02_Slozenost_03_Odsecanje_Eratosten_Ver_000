...
{
    static long[] Eratosten_Niz_Napuni_int(long n)         // ProstiBrojeviDo N
    {
        long[] Prost_niz = new long[n + 1];
        for (long b = 0; b <= n; b++) Prost_niz[b] = b;

        for (long b = 2; b * b <= n; b++)
            if (Prost_niz[b] == b)
                for (long k = b * b; k <= n; k = k + b) 
                    if (Prost_niz[k] == k)
                        Prost_niz[k] = b;
        return Prost_niz;
    }
    ...
    static void Main()
    {
        ...
        long n = 10;
        long[] Prost_niz = Eratosten_Niz_Napuni_int(n);
        for (long b = 1; b <= n; b++) Console.Write(Prost_niz[b] + " ");
    }
    ...
    static void Zad_01_Zbir_prostih_prost()
    {
        ...
        bool[] Prost_niz = Eratosten_Niz_Napuni(n);
        int brojParova = 0;
        int p = 2;
        for (int q = p + 1; p + q <= n; q++)
            if (Prost_niz[q] && Prost_niz[p + q])
                brojParova++;
